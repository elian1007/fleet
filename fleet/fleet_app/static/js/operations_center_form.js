$(document).ready(function () {
  $('#select-multiple-person').multiSelect({
    selectableHeader: "<div class='custom-header'>Personal Disponible</div>",
    selectionHeader: "<div class='custom-header'>Personal asociado</div>",
  })
    ...
    form.addEventListener('submit', event => {
      ...
      if (form.checkValidity()) {
        var data = {
          ...
          contacto: $('#contact-name').val(),
          empleados: $('#select-multiple-person').val()
        }
        ...
        Swal.fire({
          ...
          didOpen: () => {
            ...
            $.ajax({
              ...
              url: `${baseUrl}/api/operations_center${isEdit ? "/" + $('#code').val() : ''}`,
              data: JSON.stringify(data),
            }).done(function (data) {
              Swal.fire({
                title: 'Éxito',
                text: isEdit ? "Se han guardado los cambios satisfactoriamente" : "El centro de operaciones ha sido creado satisfactoriamente",
                icon: 'success',
                ...
                if (result.isConfirmed) {
                  window.location.replace(`${baseUrl}/operations_center`)
                }
              })
            }).fail(function (data) {
              ...
              if (status === 500) {
                ...
                text = "Ya existe un centro de operación registrado con este código";
              }
              Swal.fire({
                title: 'Error',
                text: text,
